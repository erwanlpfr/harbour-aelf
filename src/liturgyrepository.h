...
signals:
    ...
    void hourLiturgyChanged(const LiturgyIdentifierKey& key, HourLiturgy::HourType type, HourLiturgy* hourLiturgy);
...
public slots:
    ...
    void onHourLiturgyFetched(const LiturgyIdentifierKey& key, HourLiturgy::HourType type, HourLiturgy* hourLiturgy);
    void onErrorOccurred(const LiturgyIdentifierKey& key, const QString& error);

private slots:
    void onServiceInformationFetched(const QDate& date, const QString& zone, Information* information);
    void onServiceMassesFetched(const QDate& date, const QString& zone, QList<Mass*> masses);
    void onServiceHourLiturgyFetched(const QDate& date, const QString& zone, HourLiturgy::HourType type, HourLiturgy* hourLiturgy);
