...
HourLiturgyModel::HourLiturgyModel(QObject* parent)
    : InformationModel(parent), m_type(HourLiturgy::Vespers), m_hourLiturgy(nullptr) {
    connect(m_liturgyRepository, &LiturgyRepository::hourLiturgyChanged, this,
            &HourLiturgyModel::onHourLiturgyChanged);
}
...
void HourLiturgyModel::setType(HourLiturgy::HourType type) {
    if (m_type != type) {
        ...
        if (m_date.isValid() && !m_zone.isEmpty()) {
            setLoading(true);
            m_liturgyRepository->loadHourLiturgy(
                LiturgyIdentifierKeyFactory::getPair(m_date, m_zone), m_type);
        }
    }
}
...
void HourLiturgyModel::onHourLiturgyChanged(const LiturgyIdentifierKey& key,
...
void HourLiturgyModel::loadData() {
    m_liturgyRepository->loadHourLiturgy(LiturgyIdentifierKeyFactory::getPair(m_date, m_zone),
                                         m_type);
}
