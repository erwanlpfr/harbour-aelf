...
QString ErrorHandler::getLocalizedErrorMessage(ErrorType type, ErrorSeverity severity,
    ...
    default:
        baseMessage =
            QCoreApplication::translate("ErrorHandler", "Une erreur inconnue est survenue");
        break;
    }
...
QString ErrorHandler::getNetworkErrorMessage(const QString& details) const {
    if (details.contains("timeout", Qt::CaseInsensitive)) {
        return QCoreApplication::translate(
            "ErrorHandler", "Délai d'attente réseau dépassé. Vérifiez votre connexion.");
    } else if (details.contains("host not found", Qt::CaseInsensitive)) {
        return QCoreApplication::translate(
            "ErrorHandler", "Serveur introuvable. Vérifiez votre connexion internet.");
    } else if (details.contains("connection refused", Qt::CaseInsensitive)) {
        return QCoreApplication::translate(
            "ErrorHandler", "Connexion refusée. Le serveur est peut-être indisponible.");
    } else {
...
QString ErrorHandler::getDatabaseErrorMessage(const QString& details) const {
    if (details.contains("no such table", Qt::CaseInsensitive)) {
        return QCoreApplication::translate("ErrorHandler",
                                           "Base de données corrompue ou incomplète.");
    } else if (details.contains("unable to open", Qt::CaseInsensitive)) {
        return QCoreApplication::translate("ErrorHandler",
                                           "Impossible d'ouvrir la base de données Bible.");
    } else {
        return QCoreApplication::translate("ErrorHandler", "Erreur de base de données : %1")
            .arg(details);
    }
}
...
QString ErrorHandler::getFileSystemErrorMessage(const QString& details) const {
    return QCoreApplication::translate("ErrorHandler", "Erreur d'accès aux fichiers : %1")
        .arg(details);
}
