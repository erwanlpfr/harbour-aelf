...
Reading::Reading(ReadingType readingType, const QString& refrainPsalm, const QString& title,
                 ...
                 const QString& refrainReference, QObject* parent)
    : QObject(parent), m_readingType(readingType), m_refrainPsalm(refrainPsalm), m_title(title),
      m_content(content), m_reference(reference), m_introRead(introRead),
      m_gospelVerse(gospelVerse), m_refrainReference(refrainReference), m_editor(editor),
      m_author(author) {
}

#define READING_GETTER(Type, name, member)                                                         \
    Type Reading::name() const {                                                                   \
        return member;                                                                             \
    }
