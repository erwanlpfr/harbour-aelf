...
QList<Reading*> AelfHourLiturgyParser::parse(const QJsonObject& json, const QString& hourType,
                                             QObject* parent) {
...
Reading* AelfHourLiturgyParser::parseIntroduction(const QJsonObject& hour, QObject* parent) {
    ...
    return new Reading(Reading::Introduction, QString(), title, QString(), QString(),
                       AelfFormatter::processHtml(hour["introduction"].toString()), QString(),
...
Reading* AelfHourLiturgyParser::parsePsaumeInvitatoire(const QJsonObject& hour, QObject* parent) {
    ...
    QString content = AelfFormatter::processHtml(inv["texte"].toString() +
                                                 HtmlUtils::p(AelfTexts::gloriaPatri())) +
                      AelfFormatter::formatAntienne(antienne, AntiennePosition::After);

    return new Reading(Reading::PsaumeInvitatoire, refrainPsalm,
                       QCoreApplication::translate("AelfTexts", "Antienne invitatoire"), QString(),
                       QString(), content, reference.isEmpty() ? QString() : "Ps " + reference,
                       QString(), QString(), QString(), parent);
}
...
Reading* AelfHourLiturgyParser::parseHymne(const QJsonObject& hour, QObject* parent) {
    ...
    return new Reading(Reading::Hymne, hymne["titre"].toString(),
                       QCoreApplication::translate("AelfTexts", "Hymne"),
                       hymne["editeur"].toString(), hymne["auteur"].toString(),
                       AelfFormatter::processHtml(hymne["texte"].toString()), QString(), QString(),
...
Reading* AelfHourLiturgyParser::parseVersetPsaume(const QJsonObject& hour, QObject* parent) {
    ...
    return new Reading(Reading::VersetPsaume, QString(),
                       QCoreApplication::translate("AelfTexts", "Verset"), QString(), QString(),
                       content, QString(), QString(), QString(), QString(), parent);
}
...
QList<Reading*> AelfHourLiturgyParser::parsePsalmsAndCantiques(const QJsonObject& hour,
                                                               QObject* parent) {
    ...
    for (int i = 1; i <= 5; ++i) {
        ...
        if (hour.contains(psaumeKey) && hour[psaumeKey].isObject()) {
            ...
            QString content = AelfFormatter::processHtml(psaume["texte"].toString() +
                                                         HtmlUtils::p(AelfTexts::gloriaPatri()));
        ...
        if (hour.contains(cantiqueKey) && hour[cantiqueKey].isObject()) {
            ...
            QString content = AelfFormatter::processHtml(cantique["texte"].toString() +
                                                         HtmlUtils::p(AelfTexts::gloriaPatri()));

            psalms.append({Reading::Canticle, antienne, cantique["titre"].toString(),
                           cantique["reference"].toString(), content});
        }
    }
    ...
    for (int i = 0; i < psalms.size(); ++i) {
        ...
        QString refrainPsalm =
            isFirst ? AelfFormatter::formatAntienne(pd.antienne, AntiennePosition::Before)
                    : QString();
        QString content =
            isLast
                ? pd.content + AelfFormatter::formatAntienne(pd.antienne, AntiennePosition::After)
                : pd.content;

        readings.append(new Reading(pd.readingType, refrainPsalm, pd.title, QString(), QString(),
                                    content, pd.reference, QString(), QString(), QString(),
                                    parent));
    }
...
Reading* AelfHourLiturgyParser::parsePericope(const QJsonObject& hour, QObject* parent) {
    ...
    QString content =
        AelfFormatter::appendReponsIfPresent(pericope["texte"].toString(), hour, "repons");
    content = AelfFormatter::processHtml(content);

    return new Reading(Reading::Pericope, QString(), AelfTexts::paroleDeDieu(), QString(),
                       QString(), content, pericope["reference"].toString(), QString(), QString(),
                       QString(), parent);
}
...
Reading* AelfHourLiturgyParser::parseLecture(const QJsonObject& hour, QObject* parent) {
    ...
    QString content =
        AelfFormatter::appendReponsIfPresent(lecture["texte"].toString(), hour, "repons_lecture");
    content = AelfFormatter::processHtml(content);

    return new Reading(Reading::Lecture, QString(), title, QString(), QString(), content,
                       lecture["reference"].toString(), QString(), QString(), QString(), parent);
}
...
Reading* AelfHourLiturgyParser::parseTextePatristique(const QJsonObject& hour, QObject* parent) {
    ...
    QString content = AelfFormatter::appendReponsIfPresent(hour["texte_patristique"].toString(),
                                                           hour, "repons_patristique");
    content = AelfFormatter::processHtml(content);
    ...
    return new Reading(Reading::TextePatristique, QString(), title, QString(), QString(), content,
                       ref, QString(), QString(), QString(), parent);
}

Reading* AelfHourLiturgyParser::parseTeDeum(const QJsonObject& hour, QObject* parent) {
    QJsonObject teDeum = hour["te_deum"].toObject();

    return new Reading(Reading::TeDeum, QString(), teDeum["titre"].toString(), QString(), QString(),
                       AelfFormatter::processHtml(teDeum["texte"].toString()),
                       teDeum["reference"].toString(), QString(), QString(), QString(), parent);
}

Reading* AelfHourLiturgyParser::parseCantiqueMariale(const QJsonObject& hour, QObject* parent) {
    QJsonObject cantique = hour.contains("hymne_mariale") ? hour["hymne_mariale"].toObject()
    ...
    return new Reading(Reading::CantiqueMariale, refrainPsalm, cantique["titre"].toString(),
                       QString(), QString(), content, cantique["reference"].toString(), QString(),
                       QString(), QString(), parent);
}
...
Reading* AelfHourLiturgyParser::parseIntercession(const QJsonObject& hour, QObject* parent) {
    ...
    return new Reading(Reading::Intercession, QString(), title, QString(), QString(),
                       AelfFormatter::processHtml(hour["intercession"].toString()), QString(),
                       QString(), QString(), QString(), parent);
}
...
Reading* AelfHourLiturgyParser::parseNotrePere(const QJsonObject& hour, QObject* parent) {
    ...
    return new Reading(Reading::NotrePere, QString(), AelfTexts::notrePereLabel(), QString(),
                       QString(), AelfTexts::notrePere(), QString(), QString(), QString(),
                       QString(), parent);
}

Reading* AelfHourLiturgyParser::parseOraison(const QJsonObject& hour, QObject* parent) {
    QString content = AelfFormatter::processHtml(hour["oraison"].toString()) + HtmlUtils::spacer() +
                      AelfTexts::oraisonSuffix();

    return new Reading(Reading::Oraison, QString(), AelfTexts::oraisonLabel(), QString(), QString(),
                       content, QString(), QString(), QString(), QString(), parent);
}
